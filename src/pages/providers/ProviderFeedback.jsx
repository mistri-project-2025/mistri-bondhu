...
export default function ProviderFeedback({ uid, onClose }) {
  ...
  useEffect(() => {
    ...
    const loadMessages = async () => {
      ...
      // 📥 Inbox → messages sent BY admin to this provider
      setInbox(
        msgs.filter((m) => m.userId === uid && m.role !== PROVIDER_ROLE)
      );

      // 📤 Sendbox → messages sent BY provider
      setSendbox(
        msgs.filter((m) => m.userId === uid && m.role === PROVIDER_ROLE)
      );
    };
  ...
  const del = async (id, type) => {
    ...
    if (type === "send") {
      setSendbox((p) => p.filter((m) => m.id !== id));
    } else {
  ...
  return (
    ...
    >
      ...
      <div
        ref={listRef}
        style={{ flex: 1, overflowY: "auto", padding: "0 14px" }}
      >
      ...
      {openInbox && (
        <FullScreen title="📥 Inbox" onClose={() => setOpenInbox(false)}>
          {renderMessages(inbox, "inbox")}
        </FullScreen>
      )}
    </div>
  );
