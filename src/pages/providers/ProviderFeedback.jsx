  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#fff", zIndex: 3000, display: "flex", flexDirection: "column" }}>
      ...
      <div ref={chatRef} style={{ flex: 1, overflowY: "auto", padding: 14, background: "#f0f2f5" }}>
        {messages.map((m) => (
          <div key={m.id} style={{ textAlign: m.role === "provider"? "right" : "left", marginBottom: 8 }}>
            <span style={{ display: "inline-block", padding: "10px 14px", borderRadius: 18, maxWidth: "70%", wordBreak: "break-word", background: m.role === "provider"? "#DCF8C6" : "#fff", boxShadow: "0 1px 1px rgba(0,0,0,0.1)" }}>
              {m.message}
            </span>
          </div>
        ))}
